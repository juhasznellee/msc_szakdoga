...
-export([change_filepath_prefix/2, change_filepath_prefix_h/0]).
...
-export([elimvar/2, elimfun/2, elimmac/2,
         ...
         elimdeadcode/2, elimrec/2, transform_fun_replacement/2, transform_spawn_link_err_handle/2, transform_lta_input/2, transform_oscmd_input/2, extfun/3, expfun/2, genfun/3,
...
-export([elimvar_h/0, elimfun_h/0, elimmac_h/0, elimdeadcode_h/0, 
        transform_fun_replacement_h/0, transform_spawn_link_err_handle_h/0, transform_lta_input_h/0, transform_oscmd_input_h/0, extfun_h/0, expfun_h/0, genfun_h/0,
...
-export([start_web/0, start_web/1, stop_web/0]).
...
transform_oscmd_input_h() ->
    message("Sanitizes the os:cmd call's untrusted arguments with transformation before using it.").

%% @doc Extract function refactoring
transform_oscmd_input(File, Range)
  when (is_atom(File) or is_list(File)),
        (is_list(Range) or is_tuple(Range)) ->
    transformr(transform_oscmd_input, File, Range, []);
transform_oscmd_input(_,_)->
    message("::(modfile(), regexp() | {regexp(),index()} |{offset(),offset()})"),
    usage.
% ri:transform_oscmd_input(mod_or_file,"A+B",f).


transform_lta_input_h() ->
    message("Sanitizes the list_to_atom's untrusted arguments with transformation before using it.").

%% @doc Extract function refactoring
transform_lta_input(File, Range)
  when (is_atom(File) or is_list(File)),
        (is_list(Range) or is_tuple(Range)) ->
    transformr(transform_lta_input, File, Range, []);
transform_lta_input(_,_)->
    message("::(modfile(), regexp() | {regexp(),index()} |{offset(),offset()})"),
    usage.
% ri:transform_lta_input(mod_or_file,"A+B",f).


transform_spawn_link_err_handle_h() ->
    message("Prevents unstable spawn - link calls with transformation.").

%% @doc Extract function refactoring
transform_spawn_link_err_handle(File, Range)
  when (is_atom(File) or is_list(File)),
        (is_list(Range) or is_tuple(Range)) ->
    transformr(transform_spawn_link_err_handle, File, Range, []);
transform_spawn_link_err_handle(_,_)->
    message("::(modfile(), regexp() | {regexp(),index()} |{offset(),offset()})"),
    usage.
% ri:transform_spawn_link_err_handle(mod_or_file,"A+B",f).

transform_fun_replacement_h() ->
    message("Prevents untrusted calls with transformation.").

%% @doc Extract function refactoring
transform_fun_replacement(File, Range)
  when (is_atom(File) or is_list(File)),
        (is_list(Range) or is_tuple(Range)) ->
    transformr(transform_fun_replacement, File, Range, []);
transform_fun_replacement(_,_)->
    message("::(modfile(), regexp() | {regexp(),index()} |{offset(),offset()})"),
    usage.
% ri:transform_fun_replacement(mod_or_file,"A+B",f).
...
change_filepath_prefix_h() ->
    message("change_filepath_prefix(OldPrefix, NewPefix).\n"
            "This functions allows you to put your source files to a new directory. It updates the old paths to the given new paths. \n").

change_filepath_prefix(Old, New) ->
    ui({change_filepath_prefix, Old, New}).
...
cat_errors(File) ->
    ...
    FileHashWForms = [{Hash, tokens_to_text(Tokens, Eol)}
                        || {Hash, Tokens} <- ?FileMan:tokenize(FileText, Path)],
...
start_wx_(UserName) ->
    case code:ensure_loaded(referl_wx) of
        {error, nofile} -> io:format("\nThe wx based interface is not compiled.\n");
        _ ->
